...
#define BUT1_PIO_PIN    28                     // Index
#define BUT1_PIO_PIN_MASK (1u << BUT1_PIO_PIN)  // esse já está pronto.
...
void but_callback(void) {
	  ...
	  uint32_t msg = -100;
	  xQueueSendFromISR(xQueueLedFreq, &msg, &xHigherPriorityTaskWoken);
}

void but1_callback(void) {
	BaseType_t xHigherPriorityTaskWoken = pdFALSE;
	uint32_t msg = 100;
	xQueueSendFromISR(xQueueLedFreq1, &msg, &xHigherPriorityTaskWoken);
}
...
int main(void) {
  ...
  if (xSemaphoreBut1 == NULL)
	printf("falha em criar o semaforo1 \n");
